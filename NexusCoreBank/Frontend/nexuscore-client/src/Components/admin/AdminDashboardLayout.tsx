...
export default function AdminDashboardLayout() {
  ...
  const adminName =
    localStorage.getItem("nexus_admin_name") ?? "System Director";
  ...
  useEffect(() => {
    document.title = "System Administration | NexusCore";
  ...
  const signOut = () => {
    ...
    localStorage.removeItem("nexus_admin_name");
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-950 dark:bg-[#050b14] dark:text-white">
      ...
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-72 border-r border-slate-200 bg-white lg:flex lg:flex-col dark:border-white/10 dark:bg-[#091323]">
        <AdminSidebar adminName={adminName} onSignOut={signOut} />
      </aside>
      ...
      <div className="min-h-screen lg:pl-72">
        <header className="sticky top-0 z-30 flex min-h-20 items-center justify-between border-b border-slate-200 bg-white/95 px-4 backdrop-blur-xl sm:px-6 lg:px-8 dark:border-white/10 dark:bg-[#050b14]/95">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex h-10 w-10 items-center justify-center rounded-full bg-violet-600 text-xs font-black text-white">
  ...
  return (
    <>
      <div className="flex h-20 items-center gap-3 border-b border-slate-200 px-6 dark:border-white/10">
        ...
        <div>
          <p className="font-black">NexusCore Control</p>
          ...
          return (
            ...
            >
              {({ isActive }) => (
                <>
                  {isActive && (
                    <span className="absolute bottom-2 left-0 top-2 w-0.5 rounded-full bg-violet-600 dark:bg-violet-400" />
                  )}
      ...
      <div className="border-t border-slate-200 p-5 dark:border-white/10">
        <div className="mb-4">
          ...
          <span className="mt-3 inline-flex rounded-md border border-violet-200 bg-violet-50 px-2.5 py-1 text-[11px] font-black text-violet-700 dark:border-violet-400/20 dark:bg-violet-400/[0.08] dark:text-violet-300">
